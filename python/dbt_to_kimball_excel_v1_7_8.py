#!/usr/bin/env python3
r"""
dbt_to_kimball_excel.py  (v1.7.8)

Fix: Proper word-wrap for PascalCase/CamelCase names, and dynamic node width
so labels fit without '...' unless truly necessary. Rounded fact box retained.
"""
...
# ----------------------------
# Diagram utilities (WRAP FIX)
# ----------------------------
_CAMEL_SPLIT_RE = re.compile(r'''
    (?<=[A-Za-z])(?=[A-Z][a-z]) |   # ...aA
    (?<=[a-z])(?=[A-Z])         |   # ...aA
...
def _tokenize_label(text: str):
    """Insert spaces at camel/Pascal boundaries, then split on whitespace/underscores."""
    if not text:
        return []
    spaced = _CAMEL_SPLIT_RE.sub(' ', str(text))
    # Normalize underscores to spaces, collapse multiple spaces
    spaced = re.sub(r'[_\s]+', ' ', spaced).strip()
    return spaced.split(' ')

def _wrap_label(text: str, max_chars: int, max_lines: int = 2):
    """
    Wrap on token boundaries using camel/Pascal-aware tokens.
    Returns list of <= max_lines lines. Adds ellipsis only if unavoidable.
    """
    ...
    lines, cur, n = [], [], 0
    for tok in tokens:
        add = (len(tok) if not cur else len(tok) + 1)  # +1 for space
        if n + add > max_chars and cur:
            lines.append(' '.join(cur))
            cur, n = [tok], len(tok)
            if len(lines) >= max_lines - 1:
                # Use remaining tokens on the last line if possible
                break
        else:
            cur.append(tok); n += add
    if cur and len(lines) < max_lines:
        lines.append(' '.join(cur))

    # If there are leftover tokens, append as much as fits; else ellipsis
    consumed = sum(len(l.split()) for l in lines)
    if consumed < len(tokens):
        # Try to squeeze remaining tokens into last line if it fits partly
        last = lines[-1]
        rest = ' ' + ' '.join(tokens[consumed:])
        if len(last) + len(rest) <= max_chars:
            lines[-1] = last + rest
        else:
...
def draw_star_png(
    ...
    *,
    ...
    # ---- Fact node: rounded rect with dynamic width/height ----
    fact_lines = _wrap_label(fact_label, max_label_chars+4, 2) if wrap_labels else [fact_label]
    longest = max(len(l) for l in fact_lines)
    # convert "characters" to approx width units (empirical factor)
    ...
    def draw_dim_node(x, y, label, family):
        ...
        lab_lines = _wrap_label(label, max_label_chars, 2) if wrap_labels else [label]
        longest = max(len(l) for l in lab_lines)
        w = max(2.1, 0.12 * longest + 0.9)
        h = (1.1 if len(lab_lines) == 1 else 1.35)
        r = 0.35
        ...
        if len(lab_lines) == 1:
            ax.text(x, y, lab_lines[0], ha='center', va='center', fontsize=f_dim, zorder=3)
        else:
            y0 = y + 0.16
    ...
    def draw_spokes(positions, items):
        for (x, y, _ang), spec in zip(positions, items):
...
# ----------------------------
# Main (unchanged from 1.7.7 except default max_label_chars↑)
# ----------------------------
def main():
