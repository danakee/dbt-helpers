...
2) (Optional) Relationships tab  [default: included — disable with --no-relationships]
   • One row per dbt `relationships` test (from fact model to dimension model).
   • Useful as an audit trail of exactly what dbt tests assert.

3) Star Map tab
   • Aggregated rows: (FactModel, DimensionModel, FactFKColumn(s), DimKeyColumn(s)).
   • Reflects the *effective* graph used for diagrams (see flags below).

4) One tab per model (Dimension/Fact/Other)
   • Table meta at top (labels **without '#'** and **bold**):
       Model, Kind, Materialization, Relation, Tags, SurrogateKey, BusinessKey, PrimaryKey, Description
     - For Surrogate/Business/Primary, the **original casing** from your YAML is preserved.
     - If PrimaryKey is missing in YAML, we **fall back to SurrogateKey** for display.
   • Column grid with: Column, DataType, Nullable?, Description, Tests, IsPK?, IsFK?, Source
   • Frozen header, **auto-filter**, wrapped **Description** & **Tests**, and **green-bar banding**.

5) (Optional) Star-diagram tabs (PNG embedded; Matplotlib)
   • One per fact model. Fact is a **rounded yellow rectangle**, dimensions are **rounded light-green** rectangles.
   • Dimension *labels are bold*. Fact label bold as well.
   • FK column names are rendered along the spokes.
   • Labels wrap at Camel/Pascal boundaries only (no injected spaces).
...
• --catalog   target/catalo g.json   (required)
...
--------------------------
• --include-views + --materializations TABLE,INCREMENTAL:   combined filter logic for summary/per-model tabs.
• Exclude per-model tabs via: --exclude-sheet-prefixes / --exclude-sheet-tags / --exclude-sheet-materializations /
  --exclude-sheet-path-globs.
• --no-relationships            : omit the Relationships tab (Star Map & diagrams still generated).
• --include-missing-dims        : keep Star Map rows whose target dim isn’t in manifest (suffix “(missing)”).
                                  Diagrams still skip missing dims (robustness).
• --star-diagrams               : enable diagram tabs.
• --diagram-two-rings N         : inner ring up to N dims, rest go to outer ring.
• --diagram-no-wrap-labels      : disable label wrapping in diagrams.
• --diagram-max-label-chars INT : wrapping width per line (default 18).
• All visual styling values are centralized in small helper sections and easy to tweak.

Limitations / assumptions
-------------------------
• Relationship edges are derived from dbt `relationships` tests found in `manifest.json`.
• If a test references a dimension not present in the manifest:
  - Default: skip it in Star Map & diagrams.
  - With --include-missing-dims: include it in Star Map (suffix “(missing)”); diagram still skips.

Changelog highlights
--------------------
1.9.3
  - Added/kept thorough comments.
  - Summary/Relationships/Star Map: freeze header, blue header, bold text, auto-filter,
    sensible widths, and word-wrap in selected columns.
  - Diagrams: bold dimension labels, fact stays light yellow, single light-green color for dimensions.

1.9.1–1.9.2
  - YAML-overlay merge with **original-case** model keys for display + lowercase copies for matching.
  - PK display fallback to SK. Per-model green-bar rows, header autofilter, etc.
"""
    ...
    *,
    ...
    def draw_spokes(positions, items):
        ...
        for (x, y, _), spec in zip(positions, items):
            ...
            if spec['edge_labels']:
                # Concatenate FK column names and wrap to 2–3 lines if long
                label = ', '.join([l for l in spec['edge_labels'] if l])
                midx = rx*0.35 + cx2*0.65
                midy = ry*0.35 + cy2*0.65
                dx, dy = cx2 - rx, cy2 - ry
                L = math.hypot(dx, dy) or 1.0
                off = 0.17  # place text slightly to one side of the spoke
                ex = midx - off * dy / L
                ey = midy + off * dx / L
                if len(label) > 32:
                    parts, cur, total = [], [], 0
...
def main():
    ...
    def style_table(ws, top_header_row: int, width_map: dict, wrap_cols: list, freeze=True):
        ...
        for r in range(top_header_row+1, ws.max_row+1):
            for letter in wrap_cols:
                # find index of this column letter; using the header row to resolve -> index
    ...
    # Column width presets for the top-level overview tabs
    ...
    # Relationships tab (may get omitted later via --no-relationships)
    df_rel = pd.DataFrame([
    ...
    with pd.ExcelWriter(args.out, engine='openpyxl') as writer:
        ...
        # ----- Relationships -----
        if (not args.no-relationships) if False else (not args.no_relationships):  # keep explicit check
        ...
        for alias_lower, n in alias_to_node.items():
            ...
            # Write per-model tab
            sheet_name = safe_sheet_name(sheet_name, used_sheet_names)
