...
Version: 1.8.0  (portable Matplotlib/PNG diagrams, Star Map FK/PK columns)
...
4) **One sheet per model**
   ...
   - Frozen row so the column header stays visible; wrapped descriptions; blue header
...
5) **Optional star-diagram tabs** (one per fact)
   ...
   - Sheet name is **"Star-Fact <FactName>"** (falling back to "Star Fact <FactName>" if needed)
...
- **schemas YAML** (optional; comma-separated globs)
  ...
  - Used to overlay per-column data_type/nullable/description and per-model
...
--diagram-legend                     Add a small legend text box to diagram sheet
...
- Relationships are inferred from dbt **relationships tests** in manifest.json.
  If a dimension model referenced by a test does not exist yet, the script
  **skips** that edge (no hard failure).
- Diagrams are embedded PNGs (portable). If you need *native* Excel shapes,
  we could add a Windows-only COM mode in a future version.
...
# ----------------------------
# Boundaries: aA, letter→digit, digit→letter
