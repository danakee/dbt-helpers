...
New in v1.5 — Sheet-level exclusions
------------------------------------
Exclude certain models from **per-model sheets** without removing them from Summary/Relationships/Star Map:
  --exclude-sheet-prefixes       Comma-separated prefixes matched against alias/name (case-insensitive)
  --exclude-sheet-tags           Comma-separated tags to exclude (case-insensitive)
  --exclude-sheet-materializations  Comma-separated materializations to exclude (e.g., view,ephemeral,custom_mat)
  --exclude-sheet-path-globs     Comma-separated glob patterns matched against model `path` (e.g., models/base/*)

Examples (PowerShell):
  # Hide base/stage/log models from per-model tabs by name prefix
  python .\dbt_to_kimball_excel_v1_5.py ... `
    --exclude-sheet-prefixes base_,stage_,log_

  # Hide models tagged "stage" or "logging"
  python .\dbt_to_kimball_excel_v1_5.py ... `
    --exclude-sheet-tags stage,logging

  # Hide view-materialized models from per-model tabs
  python .\dbt_to_kimball_excel_v1_5.py ... `
    --exclude-sheet-materializations view

  # Hide anything under models/base/** and models/stage/**
  python .\dbt_to_kimball_excel_v1_5.py ... `
    --exclude-sheet-path-globs "models/base/*,models/stage/*"

All other features from v1.4 are preserved (YAML overlays, custom mats, etc.).
"""
...
def main():
    ...
    with pd.ExcelWriter(args.out, engine='openpyxl') as writer:
        ...
        for n in filtered:
            ...
            _pd.concat([_pd.DataFrame(info_rows), df], ignore_index=True).to_excel(writer, index=False, sheet_name=sheet_name)
