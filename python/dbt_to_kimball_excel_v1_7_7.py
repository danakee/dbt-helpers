#!/usr/bin/env python3
r"""
dbt_to_kimball_excel.py  (v1.7.7)

What's new vs 1.7.6
- Fact node now uses a rounded rectangle (consistent with dimensions).
- Smart word-wrapping for fact & dimension labels so long names fit.
  (configurable; on by default)
- Everything else unchanged: star diagrams, dedupe roles, two rings,
  soft palette, per-model sheets with frozen headers, etc.
"""
...
def _wrap_label(text: str, max_chars: int, max_lines: int = 2):
    """Greedy wrap on word boundaries; returns list of lines (<= max_lines)."""
    text = str(text or '')
    if not text or len(text) <= max_chars:
        return [text]
    words = re.split(r'(\s+|_+)', text)  # keep separators
    lines, cur, n = [], [], 0
    for w in words:
        wlen = len(w)
        if n + wlen > max_chars and cur:
    ...
    if cur and len(lines) < max_lines:
        lines.append(''.join(cur).strip())
    # If still too long, truncate last line with ellipsis
...
def draw_star_png(
    ...
    *,
    ...
    # ---- Fact node: rounded rect (with shadow) ----
    fact_lines = _wrap_label(fact_label, max_label_chars+2, 2) if wrap_labels else [fact_label]
    rect_w, rect_h = 2.9, 1.3 if len(fact_lines) == 1 else 1.55 + 0.55*len(fact_lines)
    shadow_offset = 0.09
    ...
    def draw_dim_node(x, y, label, family):
        ...
        if shape == 'circle':
            ...
            w, h, r = 2.1, (1.1 if len(lab_lines) == 1 else 1.45), 0.35
            ax.add_patch(FancyBboxPatch(
...
def main():
    ...
    # NEW: wrapping controls
    parser.add_argument('--diagram-no-wrap-labels', action='store_true',
                        help='Disable wrapping of fact/dimension labels inside shapes.')
    parser.add_argument('--diagram-max-label-chars', type=int, default=16,
                        help='Approx chars per line before wrapping (default 16).')
