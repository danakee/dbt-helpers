...
2.0.0
  ...
  - Kept all 1.9.x features (tab colors, formatting, star diagrams, sheet naming fix) unchanged.

1.9.4
  - Colored sheet tabs by type; fixed “_1” suffix via case-insensitive safe naming.
...
def lineage_edges(manifest):
    """Yield (from_alias_lower, to_alias_lower) for model→model deps (manifest.depends_on)."""
    edges = []
...
def main():
    ...
    TAB_BLUE   = "5B9BD5"  # Summary / Relationships / Star Map / Lineage / Pipelines / Others
    TAB_PURPLE = "C9C2F3"  # Dimension model tabs
    TAB_PEACH  = "F8CBAD"  # Fact model tabs
    TAB_GREEN  = "A9D18E"  # Diagram tabs
    ...
    # Lineage edges from manifest.depends_on
    ...
    # Pipelines df (per mart closure)
    df_pipes = pd.DataFrame(columns=['MartModel','Stages','Bases'])
    parents = defaultdict(set)  # keep for optional diagrams
    if args.lineage and edges:
        ...
        df_pipes = pd.DataFrame(rows).sort_values('MartModel')
    ...
    with pd.ExcelWriter(args.out, engine='openpyxl') as writer:
        ...
        for alias_lower, n in alias_to_node.items():
            ...
            ws.freeze_panes = f"A{start_row+2}"                         # freeze just below the grid header
            hdr = start_row + 1
        ...
        if args.lineage and args.lineage_diagrams and not df_pipes.empty:
            ...
            if plt:
                ...
                # Build alias_display→alias_lower reverse map
                disp_to_alias = {v: k for k, v in alias_to_display.items()}

                # For each mart row in pipelines, draw LR DAG
                for _, prow in df_pipes.iterrows():
                    mart_disp = prow['MartModel']
                    m_alias = disp_to_alias.get(mart_disp)
                    if not m_alias:
                        continue

                    # upstream closure to collect nodes for this mart
                    ...
                    # draw edges that are within coords
