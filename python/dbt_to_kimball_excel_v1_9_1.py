...
3) **Star Map** tab
   ...
   - Diagrams are based on the same edge list as Star Map (see below).
...
4) **One sheet per model**
   - Table meta at the top (**Model, Kind, Materialization, Relation, Tags,
     ...
     - Labels have no leading `#` and are **bold**
     - Surrogate/Business/Primary keys display in **original case** from YAML
     - If PrimaryKey not provided, it **falls back to SurrogateKey**
   - Column catalog below with DataType, Nullable?, Description, Tests, IsPK?, IsFK?, Source
   - Frozen header row, **auto-filter (sort & filter)**, wrapped **Description** & **Tests**
   - Light **green-bar** banding for readability
...
# ----------------------------
# YAML overlays (display + match)
# ----------------------------
def load_yaml_overlays(paths_csv: str):
    """
    Parse schema YAMLs to build overlay:
...
def main():
    ...
    for r in norm_rel_rows:
        ...
        # if missing + include-missing-dims: still omit from spokes; diagrams skip
    ...
    with pd.ExcelWriter(args.out, engine='openpyxl') as writer:
        ...
        for alias_lower, n in alias_to_node.items():
            ...
            pk_display_final = pk_display if pk_display else sk_display  # PK fallback to SK

            pk_match = set((meta_match.get('primary_key', []) or []))
            sk_match = set((meta_match.get('surrogate_key', []) or []))
            explicit_pk_names = {*pk_match, *sk_match}  # for PK inference

            # Columns (prefer catalog; fallback to manifest)
            rows = []
            ...
            # --- Table meta (labels without '#', display casing preserved) ---
            info_rows = [
            ...
            # Freeze just below the header row of the column grid
            ...
            # Auto-filter (adds sort & filter UI on the header row)
            ...
            # Bold the meta labels (first column of the meta block)
            for r in range(1, len(df_meta.index)+1):
                ws.cell(row=r, column=1).font = Font(bold=True)

            # 'Green-bar' alternating fill on data rows
            for r in range(hdr+1, ws.max_row+1):
                if (r - (hdr+1)) % 2 == 1:
