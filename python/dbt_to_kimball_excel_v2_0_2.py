...
def classify_tier(node, *, base_globs, stage_globs, mart_globs, base_tags, stage_tags, mart_tags):
    ...
    if _match_any_glob(path_str, mart_globs) or (tags & mart_tags):
        return 'mart'
    # --- permissive fallback: detect anywhere in the path ---
    if not base_globs and not stage_globs and not mart_globs:
        p = (node.get('path') or '').replace('\\', '/').lower()
        if '/models/base/' in p or p.endswith('/models/base'):
            return 'base'
        if '/models/stage/' in p or p.endswith('/models/stage'):
            return 'stage'
        if '/models/mart/' in p or p.endswith('/models/mart'):
            return 'mart'
    return 'other'
...
def main():
    ...
    with pd.ExcelWriter(args.out, engine='openpyxl') as writer:
        ...
        # Lineage diagrams (run even if Pipelines is empty; derive marts directly if needed)
        if args.lineage and args.lineage_diagrams:
            import importlib
            try:
                plt = importlib.import_module('matplotlib.pyplot')
            ...
            if plt:
                ...
                def _get_any(row, *names):
                    # row may be a pandas Series or a dict-like
                    if hasattr(row, 'index'):
                        it = row.index
                        getv = row.get
                    else:
                        it = row.keys()
                        getv = lambda k: row.get(k)
                    for n in names:
                        if n in it:
                            return getv(n)
                        for k in it:
                ...
                # If Pipelines is empty, derive mart list directly from tier map
                iter_rows = []
                if df_pipes.empty:
                    mart_aliases = [a for a, t in alias_to_tier.items() if t == 'mart']
                    for m in mart_aliases:
                        iter_rows.append({'MartModel': alias_to_display.get(m, m)})
                else:
                    iter_rows = [prow for _, prow in df_pipes.iterrows()]

                for prow in iter_rows:
