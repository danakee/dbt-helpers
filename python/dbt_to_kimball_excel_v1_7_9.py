#!/usr/bin/env python3
r"""
dbt_to_kimball_excel.py  (v1.7.9)

Changes vs 1.7.8:
- Diagram label wrapping: break ONLY at camel/Pascal boundaries; insert '\n'
  (no spaces added to model names). Dynamic node sizing by longest wrapped line.
- Diagram sheet name: "Star-Fact <FactName>" (hyphen). Safe fallback to "Star Fact".
"""
...
# ----------------------------
# Boundaries: aA, aA..., letter→digit, digit→letter
_CAMEL_SPLIT_RE = re.compile(r'(?<=[A-Za-z])(?=[A-Z][a-z])|(?<=[a-z])(?=[A-Z])|(?<=[A-Za-z])(?=\d)|(?<=\d)(?=[A-Za-z])')
...
def main():
    ...
    # YAML overlays (still used for per-model sheets)
    ...
    with pd.ExcelWriter(args.out, engine='openpyxl') as writer:
        ...
        # Per-model tabs (unchanged from previous version)
        for alias_lower, n in alias_to_node.items():
            db, schema, alias_disp = model_relation_identifiers(n)
