...
--------------------
1.9.6
  - Back-compat CLI aliases: `--lineage`, `--lineage-diagram(s)` now map to `--star-diagrams`.
  - Keep star-diagram temp directory alive until workbook is saved (fixes missing PNG path on save).
...
1.9.4
  - Colored sheet tabs by type; fixed unwanted “_1” suffix on sheet names; diagram/formatting polish.
...
def safe_sheet_name(base: str, used: set):
    ...
    while candidate.lower() in used_lower:
        ...
        i += 1
    used.add(candidate)  # keep original casing
    return candidate
...
def get_catalog_columns_for_model(catalog, node):
    ...
    key_no_db = f'{schema}.{alias}'.lower()
    for k, v in (catalog.get('nodes') or {}).items():
        kl = k.lower()
        if kl == key_db or kl == key_no_db:
...
def main():
    ...
    # Missing dimensions per fact (for notes on model sheets)
    ...
    # =============================================================================
    # Write Excel workbook — keep a temp dir alive for diagram PNGs until save
    # =============================================================================
    with TemporaryDirectory(prefix="dbt_star_") as tmpdir:
        used_sheet_names = set()  # will be set after overview tabs
        ...
        with pd.ExcelWriter(args.out, engine='openpyxl') as writer:
            ...
            # Seed the used sheet-name set with current workbook sheets
            ...
            for alias_lower, n in alias_to_node.items():
                ...
                # Reserve a unique sheet name ONCE
                ...
                last_col_letter = get_column_letter(8)
                ...
                # Optional "missing dims" note for FACT sheets
