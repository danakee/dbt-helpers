...
--------------------------------------------------------------------------------------------------
				...
				CASE 
                    WHEN [r].[RICDAT] IS NOT NULL AND [r].[AHCCDT] IS NOT NULL
				        THEN IIF([r].[RICDAT] >= [r].[AHCCDT_Date],
                             DATEDIFF(DAY, [r].[AHCCDT_Date], [r].[RICDAT]) - (DATEDIFF(WEEK, [r].[AHCCDT_Date], [r].[RICDAT])) * 2  + [r].RICDATDateOffSet
                                ,0)
                    WHEN [r].[AHCCDT] IS NULL THEN NULL
                    WHEN [r].[RICDAT] IS NULL AND [r].[AHCCDT] IS NOT NULL
                        THEN IIF(GETDATE() - 1 >= [r].[AHCCDT_Date],
                             DATEDIFF(DAY, [r].[AHCCDT_Date], GETDATE() - 1) - (DATEDIFF(WEEK, [r].[AHCCDT_Date], GETDATE() - 1)) * 2  + [r].CurrentDateOffSet + [r].startDateOffSet
                                ,0)
	...
	SELECT AHID
		  ,MIN(CASE WHEN ADMODT = 'SIM' AND ADSTAT IN ('S', 'P', 'C') THEN d.Date END) AS FirstSIMDate
      INTO #FirstSIMDate
	  FROM OperationsAnalyticsstage.dbo.Stage_FPCRAH ah
	  JOIN OperationsAnalyticsStage.dbo.Stage_FPCRAD ad
	    ON ADAHID = AHID
      LEFT JOIN OperationsAnalytics.dbo.dimDate d
	    ON d.Db2Date = ADSDAT
	 GROUP BY AHID
	...
	SELECT 
		...
		,CASE 
            WHEN CHARINDEX('Test', [r].[ReopenReason]) > 0 THEN 'Y'
            WHEN [OnTimeFlag_new] = 'Y' THEN 'Y'
            WHEN [OnTimeFlag_new] = 'N' AND [lkpROP].[ImpactTimelinessFlag] IS NULL THEN 'N'
            WHEN [OnTimeFlag_new] = 'N' AND [lkpROP].[ImpactTimelinessFlag] = 'Y'   THEN 'N'
            WHEN [OnTimeFlag_new] = 'N' AND [lkpROP].[ImpactTimelinessFlag] = 'N'   THEN 'Y'		
        END AS [OnTimeFlag]
        ---------------------------------------------------------------------------
        -- FSM 61.58 Effort - Add column [AHTCID] - Begin
        ---------------------------------------------------------------------------
	...
	FROM 
	    ...
	    LEFT OUTER JOIN [OperationsAnalyticsStage].[dbo].[lkpReopenReason] AS [lkpROP]
		    ON [lkpROP].[ReopenReason] = [r].[ReopenReason]
        LEFT OUTER JOIN #FirstSIMDate fsd
		    ON fsd.AHID = r.AHID
			;
...
-- when RICDAT is available

    UPDATE r 
	   SET ROTElapsedDays = r1.ROTElapsedDays
	  FROM OperationsAnalytics.dbo.factReservation r
	  JOIN 
		(
			 SELECT AHCCDT
			       ,RICDAT
				   ,AHID
                   ,CASE WHEN AHCCDT = RICDAT then 0 
	                     when RICDAT < AHCCDT then 0 
	                ELSE COUNT(DISTINCT d.Date) - 1 
					END AS ROTElapsedDays
               FROM OperationsAnalytics.dbo.factreservation r
               LEFT JOIN OperationsAnalytics.dbo.dimdate d
                 ON d.date BETWEEN AHCCDT AND RICDAT 
	          WHERE RICDAT IS NOT NULL 
...
-- if the job runs in the PM consider  today() as Record Date
    ...
    UPDATE r 
	  ...
	  JOIN 
		(
			 SELECT AHID
				   ,COUNT(d.date) AS ROTElapsedDays
			   FROM OperationsAnalytics.dbo.factReservation r
			   LEFT join OperationsAnalytics.dbo.dimdate d
		  		 ON d.date > AHCCDT
				AND d.date < CAST(GETDATE() as date)  
			  WHERE d.weekday NOT IN ('Saturday', 'Sunday')
				AND RICDAT is null
				AND DATEPART(HOUR,GETDATE()) between 12 and 23
			  GROUP BY AHID
		  ) r1
        ON r1.AHID = r.AHID

-- if the job runs in the early AM consider Yesterday (today() -1 )as Record Date
    ...
    UPDATE r 
	  ...
	  JOIN 
		(
			 SELECT AHID
				   ,COUNT(d.date) - 1 AS ROTElapsedDays
			   FROM OperationsAnalytics.dbo.factReservation r
			   LEFT JOIN OperationsAnalytics.dbo.dimdate d
		  		 ON d.date > AHCCDT
				AND d.date < CAST(GETDATE() AS DATE)  
			  WHERE d.weekday NOT IN ('Saturday', 'Sunday')
				AND RICDAT IS NULL
				AND DATEPART(HOUR,GETDATE()) BETWEEN 00 AND 11
			  GROUP BY AHID
		  ) r1
        ON r1.AHID = r.AHID  
