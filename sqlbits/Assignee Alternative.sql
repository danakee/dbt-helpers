...
SELECT
   ...
   , ROW_NUMBER() OVER (
       PARTITION BY [fk_issue]
       ORDER BY [create_dt], [prim_key]
   ...
   , [hvr_change_time]
FROM [Fsi_Issues2].[dbo].[tblnodedata]
WHERE [create_dt] > DATEFROMPARTS(2007, 1, 1)
  AND [latest] = 1
),
[Normalized] AS (
SELECT
...
SELECT
     *
   , SUM([ChangeFlag]) OVER (
       PARTITION BY [IssuePKey]
       ORDER BY [NodeCreateDateTime], [NodeDataPKey]
...
SELECT
   ...
   , r.[RunId]
   , ROW_NUMBER() OVER (
       PARTITION BY r.[IssuePKey], r.[RunId]
       ORDER BY r.[NodeCreateDateTime], r.[NodeDataPKey]
     ) AS [rn_in_run]
   ...
   , FIRST_VALUE(r.[hvr_change_time]) OVER (
       PARTITION BY r.[IssuePKey], r.[RunId]
       ORDER BY r.[NodeCreateDateTime], r.[NodeDataPKey]
       ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING
     ) AS [RunStartChangeTime]
   , LAST_VALUE(r.[hvr_change_time]) OVER (
       PARTITION BY r.[IssuePKey], r.[RunId]
       ORDER BY r.[NodeCreateDateTime], r.[NodeDataPKey]
       ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING
     ) AS [RunEndChangeTime]
...
SELECT
   ...
   , c.[RunStartChangeTime]
   , c.[RunEndChangeTime]
   , LEAD(c.[RunStartDateTime]) OVER (
       PARTITION BY c.[IssuePKey]
       ORDER BY c.[RunStartDateTime]
...
SELECT
   ...
   , [RunLastSeenDateTime]                                                 AS [IssueAssigneeLastSeenDateTime]      -- last node time within the run
   , [RunStartChangeTime]                                                  AS [IssueAssigneeStart_hvr_change_time] -- from the chosen start row
   , [RunEndChangeTime]                                                    AS [IssueAssigneeEnd_hvr_change_time]   -- from the chosen end row
