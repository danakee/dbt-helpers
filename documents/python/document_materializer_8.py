...
def _classify_storage(record: dict) -> str:
    ...
    if record.get("DocumentLink"):
        return "link"
    if record.get("UDocumentContent") is not None:
        return "db_ucontent"            # unencrypted blob in UDOCUMENT_CONTENT
...
def get_file_bytes(record: dict) -> Optional[bytes]:
    ...
    Patterns handled:
        ...
        db_ucontent         → raw bytes from UDOCUMENT_CONTENT (unencrypted)
        db_file             → bytes from DOCUMENT_FILE column (Base64 encoded)
        db_content_blowfish → bytes from DOCUMENT_CONTENT column (Blowfish encrypted)
        file_and_content    → bytes from DOCUMENT_FILE (preferred for overlap rows)
        disk_blowfish       → bytes read from PATH_OR_TABLE on disk (Blowfish encrypted)
    ...
    if pattern == "db_ucontent":
        raw = record.get("UDocumentContent")
        if raw is None:
            log.warning("  UDocumentContent is None for db_ucontent pattern")
            return None
        # UDOCUMENT_CONTENT is a SQL image column containing raw unencrypted
        # file bytes — confirmed as valid file magic (e.g. %PDF = 25504446).
        # No decryption needed.
        result = bytes(raw) if not isinstance(raw, bytes) else raw
        log.debug(
            "  UDocumentContent: %d bytes  magic=%s",
            len(result),
            result[:4].hex() if len(result) >= 4 else result.hex(),
        )
        return result
...
def materialize_document(
    ...
    for record in records:
        ...
        file_bytes = raw
        if pattern == "db_ucontent":
            # UDocumentContent is already unencrypted — skip decryption entirely
            log.debug("  db_ucontent — skipping decryption")
        elif record.get("DocumentKeyType") == "BLOWFISH":
